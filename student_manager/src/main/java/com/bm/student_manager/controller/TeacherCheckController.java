...
public class TeacherCheckController {
    ...
    public Result selectPage(@RequestParam(defaultValue = "1") int page, @RequestParam(defaultValue = "10") int limit,String courseNameSETime) {
        try {
            PageHelper.startPage(page, limit);
            List<TeacherCheck> list = teacherCheckService.selectPage(courseNameSETime);
            if (list == null) {
                return new Result().successMessage("无数据");
            } else {
                return new Result(200, "ok", list, teacherCheckService.countPage(courseNameSETime));
            }
        } catch (Exception ex) {
            return new Result().error(ex.getMessage());
        }
